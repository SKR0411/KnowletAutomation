...
<body>
    <header class="header">
    <h1>Notes and Study Meterial</h1>
...
def write(out_path, html):
    if os.path.exists(out_path):
        ...
        with open(out_path, "w") as f:
            f.write(html)
        status = '➕'
    print(status, out_path.removeprefix(root_dir).replace('.html', ''))
    return status
...
for sem, subjects in structure.items():
    for sub, papers in subjects.items():
        ...
        for pap in papers.keys():
            ...
            tags += f'            <div class="subject-card"><a href="{sub}/{pap}">{pap_title}</a></div>\n'
...
for sem, subjects in structure.items():
    ...
    for sub in subjects.keys():
        ...
        tags += f'            <div class="subject-card"><a href="{sem}/{sub}">{sub_title}</a></div>\n'
...
# 2D. Generate Notes Page 
tags = ""

for sem in structure.items():
    sem = sem[0]
    sub_title = sem.replace("_", " ").title()
    tags += f'            <div class="subject-card"><a href="notes/{sem}">{sub_title}</a></div>\n'
...
out_path = root_dir + ".html"
status = write(out_path, html)
count(status, notcount)

print('\n')
print('Notes:    [Total, New, Same, Changed] =', notcount)
print('Semester: [Total, New, Same, Changed] =', semcount)
